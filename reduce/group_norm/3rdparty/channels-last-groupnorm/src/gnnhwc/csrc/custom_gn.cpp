...
std::tuple<at::Tensor, at::Tensor, at::Tensor> gn_nhwc_fwd_v2(
  const at::Tensor X,
  const at::Tensor weight,
  const at::Tensor bias,
  const int64_t G,
...
at::Tensor X_nhwc = X.permute({0, 2, 1});
at::Tensor X_out = at::empty_like(X_nhwc);
at::Tensor means = at::zeros({N, G}, weight.options().dtype(at::kFloat));
at::Tensor rstds = at::zeros({N, G}, weight.options().dtype(at::kFloat));
...
AT_DISPATCH_FLOATING_TYPES_AND2(
  ...
  X.scalar_type(),
  "group_norm_nhwc_forward", [&]() {
  run_gn_fwd_kernels_v2<scalar_t>(
      X_nhwc.const_data_ptr<scalar_t>(),
      weight.const_data_ptr<scalar_t>(), bias.const_data_ptr<scalar_t>(),
      N, R, C, G, static_cast<scalar_t>(eps), act_fn_option,
      X_out.mutable_data_ptr<scalar_t>(), means.mutable_data_ptr<float>(), rstds.mutable_data_ptr<float>()
  );
});
return {X_out.permute({0, 2, 1}), means, rstds};
}
