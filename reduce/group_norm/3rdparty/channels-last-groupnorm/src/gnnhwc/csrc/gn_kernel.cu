...
#include "Welford.h"
#include <cub/cub.cuh>
#define MAX_THREADS_PER_BLOCK 256 // seems to work slightly better than 256/512/1024
...
inline block_params_t calc_block_params(const int ideal_num_threads, const int threads_per_row, const int snap = -1) {
  ...
  */
  ...
  if (threads_per_row < TPB) // channel number is samller
    rows_per_block = TPB / threads_per_row;
  else { // channel number is greater
    ...
    while (TPB % snap != 0 && snap % TPB != 0) { // TPB align to snap
