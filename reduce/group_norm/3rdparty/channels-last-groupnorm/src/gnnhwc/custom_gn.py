...
gn_op = load(
        ...
        sources=[
            ...
            os.path.join(module_dir, "csrc/gn_kernel_v2.cu"),
            ],
        extra_cuda_cflags=[
            '-use_fast_math',
...
class GN_NHWC_Func_V2(torch.autograd.Function):
    @staticmethod
    def forward(ctx, X: torch.Tensor, weight: torch.Tensor, bias: torch.Tensor, G: int, eps: float, activation: str):
        ctx.x_shape = X.shape

        X_flat = X.view(X.shape[0], X.shape[1], -1)
        X_out, means, rstds = torch.ops.gnop.fwd_v2(X_flat, weight, bias, G, eps, activation)
        ctx.save_for_backward(X_flat, weight, bias, means, rstds)
        ctx.G = G
        ctx.activation = activation
        return X_out.view(ctx.x_shape)
